...
export default function WeeklyPromptDetail() {
  ...
  // "add yours" on the swipe-hub prompt page pushes here with autoSubmit and
  // opens the submit flow straight away (this screen is just the passthrough).
  const didAutoSubmit = useRef(false);
  useEffect(() => {
    if (route.params.autoSubmit && prompt && !didAutoSubmit.current) {
      didAutoSubmit.current = true;
      openSubmitFlow();
      navigation.setParams({ autoSubmit: undefined });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [route.params.autoSubmit, prompt]);
                ...
                <Pressable
                  ...
                  onPress={() => {
                    if (item.art_type === 'written_form' || item.art_type === 'audio') {
                      ...
                      // 'SearchTab' only exists inside MainTabs, which the
                      // swipeB shell doesn't render — navigating to a missing
                      // route is a silent no-op, so this tap did nothing there.
                      // SwipeBStack registers UserProfile directly.
                      const profileParams = {
                      ...
                      if (navModel === 'swipeB') {
                        (navigation as any).navigate('UserProfile', profileParams);
                      } else {
